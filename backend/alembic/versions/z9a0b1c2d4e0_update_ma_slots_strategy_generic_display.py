"""Refresh ha_month_day_ma7_slots name/description (no SMA period in title)."""

from typing import Sequence, Union

from alembic import op

revision: str = "z9a0b1c2d4e0"
down_revision: Union[str, None] = "y9z0a1b2c3d5"
...
def downgrade() -> None:
    op.execute("""
        UPDATE strategies SET
            name = 'HA Month/Day + HA cross SMA(7) (slots, no fundamentals)',
            description = 'Monthly HA bullish (close > open) plus daily HA close crossing above SMA(7) '
                'on regular closes (prior bar strictly below SMA, signal bar above); no SEC fundamentals; '
                'monthly exit when month HA closes below its open; universe order for fills; '
                'portfolio_size caps concurrent longs (default 20).',
            updated_at = now()
        WHERE id = 'ha_month_day_ma7_slots';
